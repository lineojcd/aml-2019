...
def read_data(verbose=True, save_part=10):
    try:
        ...
        train_eeg1 = pd.read_csv("{}/data/train_eeg1.csv".format(dir_cur), index_col=0)
        train_eeg2 = pd.read_csv("{}/data/train_eeg2.csv".format(dir_cur), index_col=0)
        train_emg = pd.read_csv("{}/data/train_emg.csv".format(dir_cur), index_col=0)
        y = pd.read_csv("{}/data/train_labels.csv".format(dir_cur), index_col=0)
...
def lstm():
    ...
    model.add(LSTM(32, input_shape=(512, 3), dropout=0.2, recurrent_dropout=0.2, return_sequences=True))
...
pipeline = Pipeline([
    ...
    ('nn', KerasClassifier(build_fn=lstm, epochs=100, batch_size=100, verbose=1)),
], verbose=1)
...
def main():
    ...
    try:
        ...
        test_eeg1 = pd.read_csv("{}/data/test_eeg1.csv".format(dir_cur), index_col=0)
        test_eeg2 = pd.read_csv("{}/data/test_eeg2.csv".format(dir_cur), index_col=0)
        test_emg = pd.read_csv("{}/data/test_emg.csv".format(dir_cur), index_col=0)
